...
class FactorHealthSnapshot:
    ...
    action_deadline: pd.Timestamp | None = None
...
class FactorResponseMonitor:
    ...
    def update(self, trade_date: pd.Timestamp, ic_data: dict[str, float], rank_ic_data: dict[str, float] | None = None) -> list[FactorResponseRecord]:
        """更新因子IC数据并返回需要采取的行动"""
        records = []
